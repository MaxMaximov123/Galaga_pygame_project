...
def print_kills(self):
	...
	for i in range(len(self.vis_kills)):
		if self.vis_kills[i] >= 0:
			font = pg.font.Font(None, 40 * round(config.K))
			...
			intro_rect.x = (config.WIDTH // 7 * 2 + 40) * 2 * config.K
			self.game.screen.blit(self.images[i], ((config.WIDTH // 7 * 2) * 2 * config.K, self.text_coord))
			self.text_coord += intro_rect.height
			self.game.screen.blit(string_rendered, intro_rect)
		self.text_coord += 15 * 2 * config.K
	for i in [
		'_' * 30,
		...
		font = pg.font.Font(None, 30 * round(config.K))
		string_rendered = font.render(i, 1, pg.Color('white'))
		intro_rect = string_rendered.get_rect()
		intro_rect.top = self.text_coord
		intro_rect.x = (config.WIDTH // 7 * 2) * 2 * config.K
		self.text_coord += intro_rect.height
		self.game.screen.blit(string_rendered, intro_rect)
		self.text_coord += 15
	return False
